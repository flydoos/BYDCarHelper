...
6. 支持自动获取比亚迪Cookie（不需要繁琐的抓包过程，且每天01:30、13:30程序会自动续期Cookie）
...
1. [必选] 获取Cookie：登录PC版微信（不可关闭主界面，也不可退出微信），通过“比亚迪车辆助手PC版”——设置——自动获取，可以自动获取比亚迪Cookie信息
...
自动续期Cookie原理：采用模拟人工操作的方式，定时自动打开比亚迪官方网页，从而实现续期。所以，每天01:30、13:30自动续期的时候用户不可以进行其他操作，否则可能导致续期失败。
